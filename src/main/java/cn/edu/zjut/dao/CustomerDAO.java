...
public class CustomerDAO extends BaseHibernateDAO
{
    ...
    public Customer findByCustId(int id)
    {
        ...
        {
            int custId = id;
            String queryString = "from Customer as customer where customerID=" + custId;
            Query queryObject = getSession().createQuery(queryString);
    ...
    public Address findByAddrId(int id)
    {
        log.debug("finding Customer instance by id");
        try
        {
            int addrId = id;
            String queryString = "from Address as address where addressID=" + addrId;
            Query queryObject = getSession().createQuery(queryString);
